...
if resume:
  model = pickle.load(open('save_open_ai.p', 'rb'))
else:
  model = {}
...
while True:
  ...
  if done: # an episode finished
    ...
    if episode_number % 100 == 0:
      np.savetxt('reward_sum_openai.txt', np.array(list_reward_sum), delimiter=',')
      np.savetxt('running_reward_openai.txt', np.array(list_running_reward), delimiter=',')
      pickle.dump(model, open('save_open_ai.p', 'wb'))
    reward_sum = 0
